...
import { GROUP_LABELS, compactGroupCountSummary, groupCountSummary, sortSubagentRows, type SubagentViewRow } from "./view-model.js";
...
export class SubagentsViewComponent {
  ...
  render(width: number): string[] {
    ...
    if (safeWidth >= SUBAGENT_UI.wideViewMin) {
      lines.push(fg(this.theme, SUBAGENT_UI.theme.secondary, `  ${pad("Agent", this.nameWidth(safeWidth))}  ${pad("Status", 11)}  Activity`));
    }
    let previousScope: boolean | undefined;
    if (this.options.sessionJobIds && !visibleRows.some((row) => this.options.sessionJobIds!.has(row.id))) {
      lines.push(fg(this.theme, SUBAGENT_UI.theme.secondary, "This session: no jobs"));
    }
    visibleRows.forEach((row, index) => {
      if (this.options.sessionJobIds) {
        const local = this.options.sessionJobIds.has(row.id);
        if (local !== previousScope) lines.push(fg(this.theme, SUBAGENT_UI.theme.secondary, local ? "This session" : "Other sessions / unassigned"));
        previousScope = local;
      }
      lines.push(this.renderRow(row, index === this.selected, safeWidth));
    });
    if (hiddenDone > 0) lines.push(fg(this.theme, SUBAGENT_UI.theme.tertiary, `+${hiddenDone} more done`));
    lines.push("");
  ...
  private visibleRows(): SubagentViewRow[] {
    ...
    if (this.options.sessionJobIds) ordered.sort((a, b) => Number(this.options.sessionJobIds!.has(b.id)) - Number(this.options.sessionJobIds!.has(a.id)));
  ...
  private nameWidth(width: number): number {
    return Math.min(SUBAGENT_UI.nameMax, Math.max(SUBAGENT_UI.nameMin, Math.floor(width * 0.22)));
  }

  private renderRow(row: SubagentViewRow, selected: boolean, width: number): string {
    const marker = selected ? ">" : " ";
    const token = rowToken(row);
    const state = GROUP_LABELS[row.group];
    const label = token ? fg(this.theme, token, pad(state, 11)) : pad(state, 11);
    const line = width >= SUBAGENT_UI.wideViewMin
      ? `${marker} ${pad(row.name, this.nameWidth(width))}  ${label}  ${row.activity}`
      : `${marker} ${pad(row.name, Math.max(4, width - 15))}  ${label}`;
  ...
  private renderSelectedDetail(width: number): string[] {
    ...
    if (!expanded) return width < SUBAGENT_UI.wideViewMin ? [row.detail, ""] : [];
    const header = [row.name, row.stateLabel, `updated ${row.age} ago`, row.usage].filter(Boolean).join(" · ");
    const lines = [
  ...
  private renderFooter(width: number): string[] {
    ...
    const primary = row.primaryAction === "reply"
      ? row.group === "needsInput" ? "enter answer question" : "enter send task"
    ...
    const parts = ["↑↓ select", primary, row.canReply ? "d details" : undefined, stop, "esc close"];
    const lines: string[] = [];
    for (const part of parts.filter((part): part is string => Boolean(part))) {
      const last = lines.at(-1);
      if (last && visibleWidth(`${last} · ${part}`) <= width) lines[lines.length - 1] = `${last} · ${part}`;
      else lines.push(...wrapTextWithAnsi(part, width));
    }
    if (expanded) {
      lines.push(...wrapTextWithAnsi([
        row.resultFile ? "o show result path" : undefined,
        row.canAttach ? "a prepare attach command" : undefined,
        this.refreshing ? "refreshing" : "R refresh",
      ].filter(Boolean).join(" · "), width));
    }
    if (error) lines.push(...wrapTextWithAnsi(error, width));
    return lines.map((line) => fg(this.theme, SUBAGENT_UI.theme.tertiary, line));
  }
