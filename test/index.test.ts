...
function launchHistory(id: string, resumed = false) {
  return { getEntries: () => [
    { type: "message", message: { role: "assistant", content: [{ type: "toolCall", id: "launch", name: "tmux_subagent", arguments: resumed ? { action: "resume", childId: id, message: "Next" } : { agent: "scout", task: "Task" } }] } },
  ...
  try {
    ...
    for (const [policy, heartbeatState, expectedStatus, expectedPolls, savedStatus] of [
      ...
      for (const resumed of [false, true]) {
        polls = 0;
        await handlers.get("session_start")?.({}, { cwd: root, sessionManager: launchHistory(job.id, resumed) });
        assert.equal(polls, expectedPolls, "reload must restore launched and resumed children despite a saved stopped status");
